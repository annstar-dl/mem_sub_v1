...
if [ "$SHOW_OUTPUT" -eq 1 ]; then
    dsq --job-file joblist.txt --mem=5G --cpus-per-task=4 --gres=gpu:1 -t 20:00 --partition=gpu --mail-type ALL  --batch-file="${JOB_ARRAY_NAME}_${TIMESTEMP}_dsq_job.sh"
else
    dsq --job-file joblist.txt --mem=5G --cpus-per-task=4 --gres=gpu:1 -t 20:00 --partition=gpu --mail-type ALL  --batch-file="${JOB_ARRAY_NAME}_${TIMESTEMP}_dsq_job.sh" --output=/dev/null
fi