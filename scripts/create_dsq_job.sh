...
if [[ "$SHOW_OUTPUT" -eq 1 ]]; then
    dsq --job-file joblist.txt --mem=5G --cpus-per-task=4 --gpus=rtx_pro_6000_blackwell:1 -t 20:00 --partition=gpu_rtx6000 --mail-type ALL  --batch-file="${JOB_ARRAY_NAME}_${TIMESTEMP}_dsq_job.sh"
else
    dsq --job-file joblist.txt --mem=5G --cpus-per-task=4 --gpus=rtx_pro_6000_blackwell:1 -t 20:00 --partition=gpu_rtx6000 --mail-type ALL  --batch-file="${JOB_ARRAY_NAME}_${TIMESTEMP}_dsq_job.sh" --output=/dev/null
fi