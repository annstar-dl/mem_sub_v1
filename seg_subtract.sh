...
conda run -n ves_seg python "${MEMBRANE_SUBTRACTION_DIR}/color2binary.py" "$PWD/label_pseudcolor"
echo "Converted pseudo-color masks to binary masks in $PWD/labels"
# run the membrane subtraction script
conda run -n ves_seg python "${MEMBRANE_SUBTRACTION_DIR}/run.py"  -dp ${PWD} -id ${MRC_DIR} -md "labels" --in_format "mrc" --out_format "mrc" -ds $3
