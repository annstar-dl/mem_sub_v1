...
#SBATCH --output=seg_subtract_%j.log
#SBATCH --error=seg_subtract_%j.err
#SBATCH --ntasks=1
...
conda run -n ves_seg python "${MEMBRANE_SUBTRACTION_DIR}/mrc2jpg.py" "$PWD/${MRC_DIR}" -o "$PWD/images_jpg" --format "jpeg" -dsa --scale
...
if [ ${SEGMENTATION_MODEL_FORMAT} == "onnx" ]; then
    ...
    --image_path "$PWD/images_jpg/${MRC_DIR}_ds" \
